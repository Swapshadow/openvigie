...
type View = 'matrix' | 'sources' | 'documentation';
...
export default function WatchPlan() {
  ...
  return <div className="watch-plan">
    ...
    <section className="watch-plan-panel glass-panel"><header><div><span>LIVRABLE OPÉRATIONNEL</span><h2>{view === 'matrix' ? 'Matrice à six colonnes' : view === 'sources' ? 'Sources et qualification' : 'Documentation du dispositif'}</h2></div><nav aria-label="Sections du plan de veille"><button type="button" aria-pressed={view === 'matrix'} onClick={() => setView('matrix')}>Matrice</button><button type="button" aria-pressed={view === 'sources'} onClick={() => setView('sources')}>Sources et qualification</button><button type="button" aria-pressed={view === 'documentation'} onClick={() => setView('documentation')}>Documentation</button></nav></header>
